...
def duplicate(copy_number):
    read_list = [genome for x in range(copy_number)]
    return read_list
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', action='store', dest='dis', default=0,
                        type=int, help='choose from the following distribution: \
        0: copy the sample n time perfect copy \
		1: beta_distribution, 2: alpha_distribution, 3: mixed_gamma_dis \
		default: 0. If the read length drawn from the distribution is \
    ...
    with open(arg.input+'.preprocessed', 'w') as f:
        ...
        if arg.dis == 0:
            # <FLAG> make some changes HERE
            print('This is for testing only')
            print(genome)
            # length should be the length of the genome.
            print(len(genome))
            # read_length = np.array([len(genome) for x in range(arg.seq_num)])
            # print("read_length")
            # print(read_length)
            read_list = duplicate(arg.seq_num)
        else:
            if arg.dis == 3:
